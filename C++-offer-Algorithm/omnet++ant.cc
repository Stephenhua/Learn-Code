...
//
// 2012-4 L.Jacob Mariscal Fern脙隆ndez based on Copyright (C) 1992-2008 Andras Varga
//
...
class AntNet : public cSimpleModule
{
  private:
    ...
    // Local functions
    void beginDataReply(int dest, int gate); //启动到目的地dest的数据应答过程 start data  reply process to destination dest
    int bestHopsEstimation(int dest); // 向目标节点返回最佳跳转估计return best hops estimation to destination node
    int bestHopsEstimationChoices(RoutingTable & choices ,int dest,cMessage *msg);
                             // 返回具有最佳跳数估计的门到目的地和这些门在一个向量
    ...
    int bestHopsEstimationChoicesLR(RoutingTable & choices ,int dest,cMessage *msg); // for local repair 1st step
                              // 返回具有最佳跳数估计的门到目的地和这些门在一个向量
                               //return number of gates with best hops estimation to destination and these gates in a vector
    void calcAvg1hop(double d); // 调试功能，计算每一跳的平均时间debugging function to calculate time average per one hop
    ...
    bool checkFloodingLimit(cMessage *msg); //Pre:局部修复FA;还要检查当地是否有维修 Pre: local repair FA ; check also if local repair is on
    int checkLimits(int hops);
                //检查数据包的最大跳数和最大数组限制，如果正确返回0，如果最大跳数返回1，如果最大数组返回2
    ...
    void evaporationProc(int dest, int gate); //蒸发是;执行蒸发过程  Pre: evaporation is on; perform evaporation procedure
    ...
    void goodPhDiffusion(int dest, int bestHops, int origin); // 良好的信息素分布在除起点门以外的邻近区域good pheromone spread over neighbours except origin gate
    void initReactiveFA(int dest); //Pre:我们知道在实际节点上没有正的信息素值
                                    //Pre: we know no positive pheromones values on actual node start reactive FA process from actual node to destination
    bool isBetterHopsEstimation(int dest, int gate, int hops); // 返回，如果它是更好的跳转估计比实际值return if it is better hops estimation than actual value
    ...
    bool isNeighbour(int n); // 检查节点n是否与当前节点相邻check if node n is neighbouur of current node
    bool isShortestPath(int dest, int hops); //使用hops度量，用最短路径保存包 with hops metrics save packet with shortest path
                                                // (shorter or same than previous)
    bool isSrcAddress(int addr); // check if it is a source address
    void linkError(int dest, int gate); // 更新路由信息和邻接表后，从gate将错误链接到destlink error to dest from gate after update routing info and neighbour table
    bool locateNeighbor(int dest , int & outgate);
    //检查目标是否在邻接表上，并在可变出口收集到目标的gateId
    ...
    int neighbourGates(RoutingTable & gates); //存储邻居门从邻居表和返回的邻居的数量 store neighbour gates from neighbour table and return number of neighbours
    int neighbourGates(RoutingTable & gates, int origin); // skip origin gateskip origin gate
    int nextGateProBA(cMessage * msg); // 返回下一个门向前主动BA， -1表示错误，实际节点没有路由return next gate to forward Proactive BA, -1 means error, no route in actual node
    int nextTravelChoices(RoutingTable & choices, int origin, cMessage *msg); //检查旅行选择，但没有起点门 check travel choices but no origin gate
    bool noisyChannel(cPacket *pk); // 检测噪声通道detect noisy channels
    double normCost(double d); // 归一化到非零，也不是负值normalise to non zero neither negative value (for bad symmetric estimations)
    void pheromoneDiffusion(int dest, int bestHops);
    //传播路由信息到邻居关于新的最佳估计跳跃到目的地的距离
    // spread routing info to neighbours about new best estimation hops distance to destination
    void pheromoneDiffusion(int dest, int bestHops, int origin); // 除了产地门，其余都是邻居 spread over neighbours except origin gate
    ...
    int positiveGoodPhChoices(int dest,unsigned int origin,cMessage * msg);
    // 一些好的正面信息素选择跳过起点和门到以前访问的节点
    //number of good positive pheromone choices skipping origin and gates to previous visited nodes
    int positivePheromonChoices(int dest); //返回正信息素选择数 return number of positive pheromone choices
    int positivePheromonChoices(RoutingTable & choices, int dest, cMessage * msg);
    // 返回正信息素选择数，并将其(门)存储在选择数组中
    //return number of positive pheromone choices, and store it (gates) in choices array

    //此处进行分割，Aodv路由
    void routeRequest(int dest); // AODV开始路由请求过程到destAODV start route request process to dest
    bool routeRequestRERR(int dest, int origin, int next);
    //路由请求由rer发起，如果发送了RREQ本地修复，则生成本地修复RREQ true
    // route request launched by RERR, generates local repair RREQ true if RREQ local repair were sent
    int selectOneDestination();
    //选择目标地址向量
    // from destination addresses vector // Pre : not empty destAddresses vector
    int selectOneSource();
    //选择源地址向量
    // from source addresses vector // Pre : not empty srcAddresses vector
    void sendNbPacket(int k);
    //从门k的位置发送邻居数据包
    // send neighbour packet from gate k
    void sendNbPacketTo(int gate, int dest);//从k号门向dest发送邻接数据包
    // send neighbour packet from gate k to dest
    void sendProactiveBA(); //Pre:从源地址或目标地址启动,生成主动BA并启动到目的地重新计算路径
    // Pre: launched from source or destination address
    // Generate Proactive BA and launching to destination to recalculate path
    void sendProactiveFA(int dest, double cost,int hops,int origin); // 从未完成的路由发送主动FAsend proactive FA from uncompleted route
    void sendProactiveFAtoSrc(int source, double cost,int hops, int origin); //检测到目标的新路径 Pre: detected new path to destination
                                            // 发送主动FA到源地址 send Proactive FA to source addresses
    ...
    void showCost(double cost, int dest); // 显示旧的成本信息和新的成本评估到dest地址show old cost info and new cost evaluation to dest address (debug)
    void showCostTable(); // 显示时间成本表show time cost tables (debug) // (not working perfect still)
    void showChannelParams(); //显示请求通道(最近使用的)参数 display request channel (recently used) parameters
    bool showChannelParamsByPort(int port); // 显示端口请求的通道参数 display channel parameters requested by port
    void showRoutingInfo(); //显示信息素的RPTable，单个目的地(src, dest地址)和实际邻居的估计跳数和上次成本值
    // show RPTable of pheromone, estimated hops and last time cost values to singular destinations(src, dest addresses) and actual neighbours
    void showRoutingInfo(int dest);  //显示信息素的RPTable，估计的跳数和指定目的地的最后时间成本值
    // show RPTable of pheromone, estimated hops and last time cost values about specified destination
    void showRTable(); // 在AODV或静态协议中显示感兴趣的路由表信息display interesting routing table info in AODV or static protocols
    int skipOriginChoices(RoutingTable & choices, int origin);//删除源端口上的选择
    // Pre: In local repair process
                            // delete choices across origin port
    void saveIntValue(int v); // 保存到文件整数值(save to file integer value (debug)
    int symmetricGate(int g); // 返回对称的(相反的)gateIDreturn symmetric (opposite) gateID  //特殊的网络基础wmn规范Pre : special network BaseWmn specifications
    const char * toDoubleCalc(double d); //返回为数学应用程序格式化的数值 return numeric value formatted for Math applications (swap '.' to ',' notation)
    int travelChoices(RoutingTable & choices, cMessage *msg); // 返回号码和可能的目的地return number and possible destinations (in vector choices)
    void tryLocalRepair(cMessage * msg, bool & comeback); //Pre:数据包未发现dest有阳性信息素值 Pre: Data packet found no positive pheromone value to dest
                                        // 与本地维修FA一起开始本地维修程序begin local repair procedure with local repair FA
    void updateETable(int dest, unsigned int outgate); // 更新蒸发检查时间表update evaporation check time table (AntWMNet)
    void updateHTable(int dest, unsigned int outgate,int hops); // 更新AntWMNet的跳数路由表update number of hops routing table for AntWMNet
    void updateNb(int gate, int value,int hops); //更新hops(如果值不为零)，邻居表与值和发送给新邻居一个hello包由门
    // update hops (if value not zero), neighbour table with value and sent to new neighbour a hello packet by gate
    void updateRoutingInfoOnGateError(int gate); // 跨越选定门的位置上的重置表(最近未连接)reset tables on position across selected gate (recently unconnected)
                                                // 门错误，节点从输出门消失Pre: Gate error, node out of sight from output gate
    void updateRPTable (int dest, unsigned int outgate, double cost,int hops ); //更新路由概率表和AntWMNet协议的最后时间成本
    // update routing probability table and last time cost for AntWMNet protocol
    void updateTTable(int dest, unsigned int outgate, int hops, double time); //更新上次成本表update last time cost table (AntWMNet)
    bool wasHereBefore(long int id);//检查以前是否有相同的FA包通过 check if same occurrence of FA packet passed by before
...
//                   htable[10][gateDest[k]] = 0;
    ...
    if (proBa->boolValue()) { // assign correct check Time
        if (alternateCheck->boolValue() ) 
            checkingTime = (2 * checkTime->doubleValue());
        else 
            checkingTime = checkTime->doubleValue();
    }
...
//                }
            ...
            //EV << "Double test: " <<  test << " temp芒锟铰�1: " << (double) 1/ temp << " Valor a anterior nodo en tercer puerto: "<< ptable[myAddress-1] [temp]<< endl;
    }
    else // Cpant routing
    {
...
//    }
    ...
    ptable [dest] [outgate] = ptable [dest] [outgate] * coefPh->doubleValue() + ((1/(cost * 1000) )* (1 - coefPh->doubleValue())); // use cost as Travel time; added 10芒锟铰籢3 to normalize (unitary)
        ...
        // use lineal combination ; added 10芒锟铰籢3 to normalise (unitary)
...
void AntNet::handleMessage(cMessage *msg)
{
    if (msg->isSelfMessage()) {
        ...
        if (msg->getKind() == 7) { // visiting check message
           if ((vtable [visitor-1].timestamp + visitTime->doubleValue()) < simTime().dbl() && (visitor > 0)) {
            ...
            EV << "Visiting table was used recently or has no visitors registered. Last entry (if exists) was on "<< vtable [visitor].timestamp << " visiting check time: "<< visitTime->doubleValue()<< " visitors: "<< visitor << endl;
           if (visitor > 0)
               EV << "Visiting table was used recently. Last entry was on: "<< vtable [visitor-1].timestamp <<endl;
    ...
    if (pk->getSrcAddr() != myAddress) { // Packet coming from outside, to skip packets from App layer
        if (noisyChannel(pk)) {  // ERROR packet branch
            ...
            if (mySort->longValue() == 4) { // AODV
                switch (msg->getKind()) {
                    case 0: dropCounter++;
                            EV << "Data packet loss, total: "<<  dropCounter <<endl;
                            dataCounter++;
                ...
                switch (msg->getKind()) {
                    ...
                    break;
                    case 1: 
                            EV << "Neighbour packet loss, total: "<<  nbCounter++ <<endl;
    ...
    if (pk->getKind() == 1) { // hello msg -> neighbour purpose
        if (pk->getTransientNodesArraySize() == 2) { // Pheromone diffusion packets
           ...
           // 在起始门上对dest进行了新的最佳跳数估计got new best hops estimation to dest on the origin gate
           int originGateId = pk->getArrivalGateId();
           int origin = gate(originGateId)->getIndex();
           int bestHops = pk->getHopCount(); 
           int dest= pk->getDestAddr();
...
// need to create new goodPhDiffusion proc, because of strange 1 meaning 0... better to fix such misunderstanding in future
    ...
    if (destAddr == myAddress)   // packet arrived desired destination
    {
        ...
        if (pk->getSrcAddr() == destAddr) 
            return; // same destination and source, no need to check extra features
        else{
